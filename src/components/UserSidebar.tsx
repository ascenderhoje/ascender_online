          ...
          return (
            <li key={item.id}>
              <button
                ...
                className={`w-full flex items-center transition-all font-nunito text-xs ${
              ...
              >
                <Icon size={16} className={active ? 'text-ascender-yellow' : 'text-ascender-purple'} />
  ...
  return (
    <div className="relative">
      <aside className={`bg-ascender-purple-light border-r border-ascender-purple/20 min-h-screen fixed left-0 top-0 flex flex-col shadow-lg transition-all duration-300 ease-in-out ${
        ...
        <div className={`${isCollapsed ? 'p-2' : 'p-2'} border-t border-ascender-purple/20 transition-all duration-300`}>
          {!isCollapsed && pessoa && (
            <div className="mb-1 px-2">
              <p className="text-[10px] text-ascender-purple-dark/70 font-nunito">Conectado como</p>
              <p className="text-xs font-nunito font-medium text-ascender-purple-dark truncate">{pessoa.nome}</p>
            </div>
          )}
          <button
            onClick={handleLogout}
            title={isCollapsed ? 'Sair' : undefined}
            className={`w-full flex items-center transition-colors font-nunito text-xs text-ascender-purple-dark hover:bg-white/50 rounded-xl ${
              isCollapsed ? 'justify-center px-4 py-1.5' : 'gap-2.5 px-4 py-1.5'
            }`}
          >
            <LogOut size={16} className="text-ascender-purple" />
